...
import (
	"context"
	"fmt"
	"github.com/prometheus/alertmanager/notify/ims"
	"net"
	"net/http"
	"net/url"
	"os"
...
func buildReceiverIntegrations(nc *config.Receiver, tmpl *template.Template, logger log.Logger) ([]notify.Integration, error) {
	...
	for i, c := range nc.IMSConfigs {
		add("wechat", i, c, func(l log.Logger) (notify.Notifier, error) { return ims.New(c, tmpl, l) })
	}
	if errs.Len() > 0 {
		return nil, &errs
	}
